...
class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        for (let x = 0; x < 3000; x += 650) { // creates a repeating background by adding multiple instances of the 'bg' image across the level, spaced 155 pixels apart
            let bg = this.add.image(x, 125, 'bg').setScale(5); // (x, y, key) 
            bg.setScale(6); // (scale) scales the background image to fit the desired size for the level, ensuring it covers the entire area without distortion
        }
        ...
        let enemy1 = this.enemies.create(600, 245, 'enemy_walk1');
        enemy1.setScale(.09);
        ...
        enemy2.setScale(.09);
        enemy2.body.setSize(enemy1.width * 0.6, enemy1.height * 0.8);
        enemy2.minX = 850;
        enemy2.maxX = 1030;
        enemy2.setVelocityX(-90);
        enemy2.setCollideWorldBounds(true);
        enemy2.body.onWorldBounds = true;
        

        let enemy3 = this.enemies.create(1390, 220, 'enemy_walk1');
        enemy3.setScale(.09);
        enemy3.body.setSize(enemy1.width * 0.6, enemy1.height * 0.8);
        enemy3.minX = 1150;
        enemy3.maxX = 1450;
        enemy3.setVelocityX(-150);
        enemy3.setCollideWorldBounds(true);
        enemy3.body.onWorldBounds = true;
...
this.ruins.setOrigin(0.5, 1);
        ...
        this.dialogues = [
            "Danao City, located along the northeastern coast of Cebu, has long been a center of trade and coastal livelihood.",
            "The city became known for its fishing communities, local industries, and hardworking people.",
            "Many families in Danao relied on fishing, farming, and small businesses to support their daily lives.",
            "With its rich history and strong traditions, Danao City continues to preserve the culture and spirit of northern Cebu."
        ];
...
class EndScene extends Phaser.Scene {
    ...
    // Fetch AI content
    const funFact = await generateFunFact('Danao, Cebu');
